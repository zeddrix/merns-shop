...
8. **Cold start:** free tier sleeps after ~15 min idle; first request may take 30–60s. The SPA shell loads first; catalog API calls may hang while the instance wakes. The UI shows a conditional `slow-server-banner` after ~3s of pending catalog fetches (product list, carousel, PDP, filters meta, cart rehydrate — see `SlowServerBanner.tsx` / `useSlowServerNotice.ts`). After the first slow load in a browser session, `sessionStorage` marks the server warm and suppresses repeat banners. Homepage uses one consolidated `home-catalog-loader` instead of stacked spinners. E2E reproduces this UX via `registerDelayedCatalogApi` in `tests/e2e/smoke/slow-server-notice.e2e.test.ts` — not a real Render spin-down.
